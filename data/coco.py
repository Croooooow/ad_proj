...
class COCODetection(data.Dataset):
	...
	def __init__(self, image_path, info_file, transform=None,
		...
		if self.wall_only:
			catIds = self.coco.getCatIds(catNms=['wall-concrete','wall-brick','wall-other','wall-panel','wall-stone','wall-tile','wall-wood','wall-other'])
			self.wall_ids = catIds
			print('wall id:',self.wall_ids)
			imgIds = []
			for catId in catIds:
				imgIds = imgIds + self.coco.getImgIds(catIds=catId)
				print('wall total: ', len(imgIds))
			self.ids = imgIds
		else:
	...
	def pull_item(self, index):
		...
		if self.has_gt:
			...
			if self.wall_only:
				target_ = []
				for ann in target:
					if ann['category_id'] in self.wall_ids:
						target_.append(ann)
				target = target_
		else:
			target = []
		...
		for x in crowd:
			x['category_id'] = -1
#		print([x['category_id']for x in target])
