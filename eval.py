...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str='', return_mask=False):
    """
    ...
    if return_mask:
        return img_numpy, masks.byte().cpu().numpy()
    return img_numpy
...
def evalvideo_ad(net:Yolact, path:str, out_path:str=None):
    
    t_start = time.time()
    fn = path[path.rfind('/')+1:-4]
    debug = False
    ...
    if out_path is not None:
        ...
        out_masked = cv2.VideoWriter(out_path[:-4]+"_mask.mp4", cv2.VideoWriter_fourcc(*"mp4v"), target_fps, (frame_width, frame_height))
    ...
    while status:
        ...
        if not status:
            break
        frames.append(frame)
        if extract_clip is True:
            tmp = cv2.cvtColor(frame,cv2.COLOR_BGR2GRAY)
            hist = cv2.calcHist([tmp], [0], None, [256], [0, 256])
            hists.append(hist)

    clip_left = 0
    clip_right = len(frames) - 1

    if extract_clip is True:
        score = [1] # similarity between current and last frames
        for i in range(1, len(hists)):
            score.append(cv2.compareHist(hists[i-1], hists[i], cv2.HISTCMP_BHATTACHARYYA))
        
        score = [1 if s < 0.05 else 0 for s in score]
        max_len = 0 # extract the clip with maximum length
        l = 0
        for i in range(1, len(score)):
            if score[i] == score[i-1] and score[i] == 1:
                l += 1
            else:
                if l > max_len:
                    max_len = l
                    ...
                    clip_right = i  
                l = 0
    print("processing from {} to {}.".format(clip_left, clip_right))
    print()
    
    masks = [] # storing the binary masks generated by the model
    masked_img = [] # storing the images covered with masks
    for i in range(len(frames)):
        
        if i < clip_left or i > clip_right: # we only process the select clip
            masks.append(None)
            masked_img.append(frames[i])
            continue
        ...
        img_numpy, mask = prep_display(preds, frame, None, None, undo_transform=False, return_mask=True)
        masks.append(mask)
        masked_img.append(img_numpy)

    anchors, rects = find_rects(fn, frames, masks) 

    # copy the ad image into the original video frames.
    for i in range(len(frames)):
        frame = add_ad(frames[i], rects[i])
        out.write(frame)
        frame = add_ad(masked_img[i], rects[i], anchors[i])
        out_masked.write(frame)
    ...
    if args.image is not None:
        ...
        if '::' in args.video:
            ...
            for fn in os.listdir(inp):
                print("evaluate %s:\n"%fn )
                evalvideo_ad(net, inp+'/'+fn, out+'/'+fn)	
        elif ':' in args.video:
            inp, out = args.video.split(':')
